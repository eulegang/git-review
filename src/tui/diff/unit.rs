...
use super::*;
use crate::{model::{Delta, LineStatus}, syntax::Syntax, tui::render_stateful};
...
fn widget<'a>(delta: &'a Delta, hidden_hunks: &'a [usize], theme: &'a Theme, syntax: &'a Syntax) -> Diff<'a> {
    Diff {
...
fn renders_hunk_headers() {
    ...
    let buf = render_stateful(
        ...
        DiffState { line: 0, scroll: 0, center_line: false },
    );
...
fn continues_rendering_across_hunks() {
    ...
    let delta = Delta::from_test_hunks(vec![
        ...
        ("@@ -2 +2 @@", vec![(LineStatus::Remove, "-second".to_string())]),
    ]);
    ...
    assert_eq!(buf[(166, 3)].bg, Color::Red);
}
...
fn skips_hidden_hunks() {
    ...
    let hidden = [0];
    let delta = Delta::from_test_hunks(vec![
        ("@@ -1 +1 @@", vec![(LineStatus::Add, "+hidden".to_string())]),
        ("@@ -2 +2 @@", vec![(LineStatus::Remove, "-visible".to_string())]),
    ]);
...
fn renders_line_content_and_status_backgrounds() {
    ...
    let buf = render_stateful(
        widget(&delta, &[], &theme, &syntax),
        DiffState { line: 99, scroll: 0, center_line: false },
    );

    let x = centered_x("-removed");

    assert_eq!(buf[(x, 1)].symbol(), " ");
    assert_eq!(buf[(x + 1, 1)].symbol(), "c");
    ...
    assert_eq!(buf[(0, 4)].bg, Color::Gray);
    assert_eq!(buf[(166, 4)].bg, Color::Gray);
}
...
fn applies_selected_added_foreground() {
    ...
    assert_eq!(buf[(x, 1)].bg, Color::White);
...
fn syntax_highlights_rust_diff_lines() {
    ...
    assert_eq!(buf[(x, 1)].symbol(), "f");
    assert_eq!(buf[(x, 1)].fg, Color::Magenta);
}
...
fn scrolls_to_keep_selected_line_visible() {
    ...
    let state = DiffState { line: 49, scroll: 0, center_line: false };
...
fn centers_selected_line_even_at_file_end() {
    ...
    let state = DiffState { line: 49, scroll: 0, center_line: true };
    let buf = render_stateful(widget(&delta, &[], &theme, &syntax), state);
